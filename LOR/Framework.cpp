...
void Framework::ChangeScene() {
	if (m_Scene[m_nowScene]->GetIsDestory()) {
		if (SceneName::Login == m_Scene[m_nowScene]->m_nextScene) {
			...
			if(m_nowScene == 2)
				m_Scene[3] = new RoomScene(m_hwnd, &m_network, false);
			else if (m_nowScene == 4)
				m_Scene[3] = new RoomScene(m_hwnd, &m_network, true);
			...
			m_Scene[4] = new InGameScene(m_hwnd, &m_network);
			
			m_Scene[m_nowScene]->~Scene();
			m_nowScene = 4;
		}
		else if (SceneName::InGameGuest == m_Scene[m_nowScene]->m_nextScene) {
			m_Scene[4] = new InGameSceneGuest(m_hwnd, &m_network);
