...
UpdateRoomStruct roomInfoGuest;
bool gameStart;
...
DWORD WINAPI RecvDataGuest(LPVOID arg)
{
	...
	while (running)
	{
		...
		switch (op)
		{
			// InRoom 통신
		case UPDATE_ROOM:
			retval = recvn(sock_info.sock, (char*)&roomInfoGuest, sizeof(UpdateRoomStruct), 0);
			break;
		case NOTIFY_CHANGEMAP:
			retval = recvn(sock_info.sock, (char*)&roomInfoGuest.m_mapInfo, sizeof(roomInfoGuest.m_mapInfo), 0);
			break;
		case PERMIT_CHANGECHAR:
		{
			PermitChangeCharStruct chaInfo;
			retval = recvn(sock_info.sock, (char*)&chaInfo, sizeof(chaInfo), 0);
			roomInfoGuest.m_charInfo[chaInfo.m_idx] = chaInfo.m_charInfo;
			break;
		}
		case PERMIT_EXIT:
