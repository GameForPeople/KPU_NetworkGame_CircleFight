...
void RoomSceneGuest::Draw(HDC hdc) {
	if (roomInfoGuest.m_mapInfo == MapName::Sea)
		m_backImg[0].StretchBlt(hdc, 0, 0, SCREEN_WIDTH - 15, SCREEN_HEIGHT - 40, SRCCOPY);
	else if (roomInfoGuest.m_mapInfo == MapName::Forest)
...
void RoomSceneGuest::DrawCharacter(HDC hdc) {
	...
	for (int i = 0; i < 4; ++i)
	{
		if (roomInfoGuest.m_charInfo[i] == CharacterName::NONE) continue;
		x = GetXPosByIdx(i);	y = GetYPosByIdx(i);
		if (roomInfoGuest.m_charInfo[i] == CharacterName::Archer)
			m_ImgArcher.TransparentBlt(hdc, x, y, 200, 500, RGB(252, 252, 252));
		else if (roomInfoGuest.m_charInfo[i] == CharacterName::Zombie)
			m_ImgZombie.TransparentBlt(hdc, x, y, 200, 500, RGB(252, 252, 252));
		else if (roomInfoGuest.m_charInfo[i] == CharacterName::Knight)
			m_ImgKnight.TransparentBlt(hdc, x, y, 200, 500, RGB(252, 252, 252));
		else if (roomInfoGuest.m_charInfo[i] == CharacterName::Wicher)
